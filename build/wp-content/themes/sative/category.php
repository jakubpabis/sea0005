		...
		<div class="row">
			<div class="col">
				<h1 class="text-size-xxxxlarge">
					<?php echo pll_e('Category'); ?>: <?php echo single_cat_title(); ?>
				</h1>
			</div>
		</div>
		<div class="row align-items-center justify-content-between header__jobs-search knowledge">
			...
			<div class="col-12 text-right">
				<p class="text-size-small font-primary">
					<span class="jobsno"><?php echo $post_no; ?></span>
					<?php
								...
								<div class="row">
									...
									<?php if (!empty($cats)) : ?>
										<h4>
											<?php $i = 0;
											foreach ($cats as $cat) : ?>
												<?php echo $i === 0 ? null : '&nbsp;|&nbsp;' ?><a href=""><?php echo $cat->name; ?></a>
											<?php $i++;
											endforeach; ?>
										</h4>
									<?php endif; ?>
									...
									<a href="<?php echo get_the_permalink(); ?>" class="btn btn__medium navy"><?php pll_e('Read more'); ?></a>
									<?php if (has_post_thumbnail()) : ?>
									</div>
								</div>
							<?php endif; ?>
					...
					<form action="" method="GET" id="filter">
						...
						<?php foreach ($categories as $cat) : ?>
							<?php if ($cat->parent === $know->term_id) : ?>
								...
								?>
								<div class="filter-group <?php echo $active ?>">
									<span class="filter-title <?php echo $active ?>">
										<?php echo $cat->name; ?>
										<i class="fas fa-plus"></i>
										<i class="fas fa-minus"></i>
										<input type="checkbox" name="category[]" value="<?php echo $cat->term_id; ?>" <?php echo $checked; ?> />
									</span>
								</div>
							<?php endif; ?>
						<?php endforeach; ?>
